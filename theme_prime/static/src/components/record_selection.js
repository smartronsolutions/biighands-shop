...
export class TpRecordSelector extends AbstractComponent {
    ...
    get themePrimeConfig() {
        let { _getDrthemeConfig } = this.props.extras;
        return _getDrthemeConfig() || { };
    }
    get hasSwitcher() {
    ...
    get resModelInfo() {
        return {
            'product.template': { title: _t("Products") }, 'product.ribbon': { title: _t("Labels") }, 'product.tag': { title: _t("Tags") }, 'product.product': { title: _t("Variants") }, 'product.public.category': { title: _t("Categories") }, 'product.attribute.value': { title: _t("Brand")}};
    }
