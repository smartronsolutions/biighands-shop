...
export class TpDropDown extends Component {
    ...
    get _dropDownClass() { return this.props._dropDownClass || ''; }
...
export class TpSearchInput extends CoreComponent {
    ...
    get resModelInfo() {
        ...
        'product.ribbon': { title: _t("Labels")},
...
export class TpRangeInput extends Component {
    ...
    get maxValue() {
        // For example in snippet there is config which already have 20 items to display
        // now from theme config record limit is set to 10 in that case range input will be
        // capped at 10
        return this.value > this.props.maxValue ? this.value : this.props.maxValue || 6;
    }
...
export class TpDomainComponent extends AbstractComponent {
    setup() {
        ...
        if (props.themePrimeConfig) {
            delete props['themePrimeConfig'];
        }
        this.value = { ...props };
    ...
    _prepareRecords(index) {
        ...
        return {
            ...
            name: index,
            extras: { _getDrthemeConfig: this._getDrthemeConfig.bind(this), isReadOnly: this.props.isReadOnly, fields: ['name'], mode: 'badge', isBrand: this.getRecord(this.fieldsList, field, 'name').relationModel === 'product.attribute.value' && field === 'dr_brand_value_id' ? true : false }
        }
    }
