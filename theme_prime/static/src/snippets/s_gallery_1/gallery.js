...
export class TpImageGallery extends Interaction {
    static selector = ".tp-image-gallery";
    dynamicContent = {
        "img:not(.tp-hotspot-media": {
    ...
    setup() {
        this.modalEl = null;
        this.originalSources = [...this.el.querySelectorAll("img:not(.tp-hotspot-media")].map(img => img.getAttribute("src"));
    }
    ...
    onClickImg(ev) {
        ...
        let imageEls = this.el.querySelectorAll("img:not(.tp-hotspot-media)");
        const currentImageEl = clickedEl.closest("img:not(.tp-hotspot-media)");
        ...
        const lightboxTemplate = "website.gallery.s_image_gallery_mirror.lightbox";
...
registry.category("public.interactions").add("theme_prime.tp_image_gallery", TpImageGallery);
