...
class ProductTemplate(models.Model):
    ...
    def _search_get_detail(self, website, order, options):
        ...
        if ratings:
            ...
            rating_product_ids = []
            for rating in [int(value) for value in ratings.split(',')]:
                rating_product_ids.extend([item['res_id'] for item in result if item['rating'] >= rating])
    ...
    def _get_additional_configurator_data(self, product_or_template, date, currency, pricelist, **kwargs):
        data = super()._get_additional_configurator_data(product_or_template, date, currency, pricelist, **kwargs)
        data['extraInfo'] = {
            ptav.id: {
                'dr_thumb_image': ptav.dr_thumb_image,
            } for ptav in product_or_template.attribute_line_ids.product_template_value_ids
        }
        return data

    def _get_combination_info(self, combination=False, product_id=False, add_qty=1.0, uom_id=False, only_template=False):
        combination_info = super()._get_combination_info(combination=combination, product_id=product_id, add_qty=add_qty, uom_id=uom_id, only_template=only_template)
        website = self.env['website'].get_current_website()
        ...
        if website and website_has_theme_prime:
            ...
            if combination_info['product_id']:
                ...
                if website._get_dr_theme_config('bool_show_bulk_price') and combination_info.get('has_b2b_access') and combination_info.get('is_combination_possible'):
                    ...
                    bulk_price = []
                    all_rule_ids = request.pricelist._get_applicable_rules(product_variant_id, fields.Datetime.now()).filtered(lambda x: x.min_quantity)
